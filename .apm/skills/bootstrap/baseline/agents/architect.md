...
**3. ⏳ TESTING TROPHY (inverted pyramid)** — *test behavior, not implementation.* Prioritize Integration & E2E over fine-grained Unit ("confidence over isolation"). **Scales with `<TOOL_DIR>/army.conf`:** at `TEST_POLICY=none` the blueprint plans NO tests and drops the auto-critic TDD block (Rule 10); at `light`/`pragmatic` plan a reduced mix. Never scale down the security/contract rigor.
