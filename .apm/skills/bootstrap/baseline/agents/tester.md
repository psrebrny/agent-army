...
## Edge cases
- **Project policy** (`<TOOL_DIR>/army.conf`): at `TEST_POLICY=none` you should not be invoked at all (say so if you are); at `light`/`pragmatic` write thin happy-path coverage and drop strict RED-first — don't re-impose full TDD against the repo's chosen level.
