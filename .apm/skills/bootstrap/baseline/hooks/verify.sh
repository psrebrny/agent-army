...
[ "${TEST_POLICY:-strict}" = none ] && echo "verify> TEST_POLICY=none — tests NOT enforced (project policy, army.conf)"
[ "${LINT_POLICY:-on}" = off ]      && echo "verify> LINT_POLICY=off — lint NOT enforced (project policy, army.conf)"
