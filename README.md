...
/bootstrap
```
Already-specialized agents are your repo's files — re-running `/bootstrap` refreshes them in place. Commit before a re-run so `git diff` / `git checkout` is your rollback (no backup files are left behind).
